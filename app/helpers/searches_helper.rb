module SearchesHelper
  ...
  def render_save_or_clear_link search
    if current_or_guest_user && search.saved?
      button_to t('blacklight.search_history.forget'), forget_search_path(search.id), :class => "btn btn-primary btn-mini"
    else
      button_to t('blacklight.search_history.save'), save_search_path(search.id), :method => :put, :class => "btn btn-primary btn-mini"
    end
  end
